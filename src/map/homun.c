...
int homun_get_create_homunid()
{
	int i;
	while(embryo_count){
		i = atn_rand()%homun_count;
		if(atn_rand()%1000000 < embryo_data[i].per)
...
static int homun_natural_heal_hp(int tid,unsigned int tick,int id,int data)
{
	...
	nullpo_retr(0, bl);
	nullpo_retr(0,(hd=(struct homun_data *)bl));
...
int read_homundb(void)
{
	...
	for(i=0;i<2;i++){
		...
		while(fgets(line,1020,fp)){
			...
			homun_db[j].script = parse_script(np,filename[i],lines);
			homun_count++;
		}
		fclose(fp);
		printf("read %s done (count=%d)\n",filename[i],homun_count);
	}
	return 0;
}
//
