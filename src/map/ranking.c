...
int ranking_display_point(struct map_session_data * sd,int ranking_id)
{
	...
	if(ranking_id<0 || MAX_RANKING <= ranking_id)
		return 0;
	sprintf(output,"%s様の %s POINT は%dです",sd->status.name,ranking_title[ranking_id],sd->ranking_point[ranking_id]);
...
int ranking_display(struct map_session_data * sd,int ranking_id,int begin,int end)
{
	...
	sprintf(output,"=========== %s RANK ==============",ranking_title[ranking_id]);
	clif_displaymessage( sd->fd,output);

	for(i = begin;i<=end;i++)
	{
		sprintf(output,"[ %2d ] %16s  :%6d POINT",i+1,ranking_data[ranking_id][i].name,ranking_data[ranking_id][i].point);
		clif_displaymessage( sd->fd, output);
	}
	sprintf(output,"===========================================");
	clif_displaymessage( sd->fd, output);
