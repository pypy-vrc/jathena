...
int party_created(int account_id,int fail,int party_id,char *name)
{
	...
	nullpo_retr(0, sd=map_id2sd(account_id));
...
// 経験値公平分配
int party_exp_share(struct party *p,int map,atn_bignumber base_exp,atn_bignumber job_exp)
{
	struct map_session_data *sd;
	int i,c;
	...
	{
		if((sd=p->member[i].sd)!=NULL && sd->bl.m==map && !unit_isdead(&sd->bl) && sd->bl.prev != NULL &&
			(!sd->sc_data ||
