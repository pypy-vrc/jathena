...
static int unit_walktoxy_timer(int tid,unsigned int tick,int id,int data)
{
	...
	if(ud->walkpath.path_half==0){ // マス目中心へ到着
		...
		if(sd) {
			...
			map_foreachinmovearea(mob_ai_hard_spawn_sub,bl->m,x-AREA_SIZE*2,y-AREA_SIZE*2,x+AREA_SIZE*2,y+AREA_SIZE*2,dx,dy,BL_PC,md,0);
		} else if(pd) {
		...
		if(sd) {
			...
			map_foreachinmovearea(mob_ai_hard_spawn_sub,bl->m,x-AREA_SIZE*2,y-AREA_SIZE*2,x+AREA_SIZE*2,y+AREA_SIZE*2,-dx,-dy,BL_PC,md,1);
		} else if(pd) {
...
int unit_movepos(struct block_list *bl,int dst_x,int dst_y,int flag)
{
	...
	if(sd) {	/* 画面外に出たので消去 */
		...
		map_foreachinmovearea(mob_ai_hard_spawn_sub,bl->m,bl->x-AREA_SIZE*2,bl->y-AREA_SIZE*2,bl->x+AREA_SIZE*2,bl->y+AREA_SIZE*2,dx,dy,BL_PC,md,0);
	} else if(pd) {
	...
	if(sd) {	/* 画面内に入ってきたので表示 */
		...
		map_foreachinmovearea(mob_ai_hard_spawn_sub,bl->m,bl->x-AREA_SIZE*2,bl->y-AREA_SIZE*2,bl->x+AREA_SIZE*2,bl->y+AREA_SIZE*2,-dx,-dy,BL_PC,md,1);
	} else if(pd) {
	...
	if(sd && sd->status.party_id>0){	// パーティのＨＰ情報通知検査
		struct party *p=party_search(sd->status.party_id);
		if(p!=NULL){
			int flag=0;
...
//	struct unit_data        *target_ud = NULL;
	...
	if( BL_CAST( BL_PC,  src, src_sd ) ) {
		...
		src_ud = &src_pd->ud;
	} else if( BL_CAST( BL_HOM, src, src_hd ) ) {
		src_ud = &src_hd->ud;
	}
	...
	} else if( BL_CAST( BL_HOM, target, target_hd ) ) {
		target_ud = &target_hd->ud;
	}
	*/
...
int unit_attack_timer_sub(int tid,unsigned int tick,int id,int data)
{
	...
	if( src_md ) {
		...
		if( !(mode&0x80) )
			return 0;
		if( !(mode&0x20) ) {
			if( tsc_data && (tsc_data[SC_HIGHJUMP].timer != -1 || tsc_data[SC_WINKCHARM].timer != -1) )
				return 0;
			if( target_sd ) {
	...
	if(src_sd && (src_sd->status.weapon != 11 && !(src_sd->status.weapon>16 && src_sd->status.weapon<22))) range++;
...
//		mobskill_deltimer(md);
		...
		struct homun_data *hd       = (struct homun_data*)bl;
		if(hd->natural_heal_hp != -1 || hd->natural_heal_sp != -1)
			homun_natural_heal_timer_delete(hd);
