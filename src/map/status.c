...
//ペコ騎乗時増えるよう移動
	...
	if(sd->sc_count && sd->sc_data){
		...
		if(sd->sc_data[SC_CHASEWALK].timer!=-1)/*チェイスウォークによる速度変化*/
		{
			if(sd->sc_data[SC_ROGUE].timer==-1)
				sd->speed -= sd->speed*(5*sd->sc_data[SC_CHASEWALK].val1)/100;
		}
	...
	if(sd->sc_count){
		...
		if(sd->sc_data[SC_ADRENALINE2].timer != -1 && sd->sc_data[SC_TWOHANDQUICKEN].timer == -1 && sd->sc_data[SC_ONEHAND].timer == -1 &&
			sd->sc_data[SC_QUAGMIRE].timer == -1 && sd->sc_data[SC_DONTFORGETME].timer == -1) {	// アドレナリンラッシュ2
		...
		if(sd->sc_data[SC_SPEARSQUICKEN].timer != -1 && sd->sc_data[SC_ADRENALINE].timer == -1 && sd->sc_data[SC_ADRENALINE2].timer == -1 &&
			...
			sd->sc_data[SC_QUAGMIRE].timer == -1 && sd->sc_data[SC_DONTFORGETME].timer == -1)	// スピアクィッケン
			aspd_rate -= sd->sc_data[SC_SPEARSQUICKEN].val2;
			
		if(sd->sc_data[SC_ASSNCROS_].timer!=-1 && // 夕陽のアサシンクロス
			sd->sc_data[SC_TWOHANDQUICKEN].timer==-1 && sd->sc_data[SC_ONEHAND].timer==-1 &&
...
int status_get_hp(struct block_list *bl)
{
	...
	else if(bl->type==BL_PC && (struct pet_data *)bl)	//changed by フェルシア
		return ((struct pet_data *)bl)->hp;
	else
		return 1;
}
/*==========================================
...
int status_get_flee(struct block_list *bl)
{
	...
	if(sc_data) {
		...
		if(sc_data[SC_INCFLEE].timer!=-1 && bl->type != BL_PC) //速度強化
			flee += 50*sc_data[SC_INCFLEE].val1;
			//THE SUN
...
int status_get_atk(struct block_list *bl)
{
	...
	if(sc_data) {
		...
		if(sc_data[SC_EXPLOSIONSPIRITS].timer!=-1 && bl->type != BL_PC)
			atk += (1000*sc_data[SC_EXPLOSIONSPIRITS].val1); //ＮＰＣ爆裂波動
...
int status_get_atk2(struct block_list *bl)
{
	...
	if(bl->type==BL_PC && (struct map_session_data *)bl)
		...
		if(sc_data) {
			...
			if(sc_data[SC_EXPLOSIONSPIRITS].timer!=-1 && bl->type != BL_PC)
				atk2 += (1000*sc_data[SC_EXPLOSIONSPIRITS].val1); //ＮＰＣ爆裂波動
...
int status_get_dmotion(struct block_list *bl)
{
	...
	if(bl->type==BL_MOB && (struct mob_data *)bl){
		...
		if(battle_config.pet_damage_delay_rate != 100)	//added by フェルシア
			ret = ret*battle_config.pet_damage_delay_rate/100;
	}else
		return 2000;
...
struct status_change *status_get_sc_data(struct block_list *bl)
{
	...
	else if(bl->type==BL_PET && (struct pet_data *)bl)	//changed by フェルシア
		return ((struct pet_data *)bl)->sc_data;
	return NULL;
}
short *status_get_sc_count(struct block_list *bl)
{
	...
	if(bl->type==BL_MOB && (struct mob_data *)bl)
		...
		return &((struct pet_data *)bl)->sc_count;
	return NULL;
}
short *status_get_opt1(struct block_list *bl)
{
	...
	if(bl->type==BL_MOB && (struct mob_data *)bl)
		...
		return &((struct pet_data *)bl)->opt1;
	return 0;
}
short *status_get_opt2(struct block_list *bl)
{
	...
	if(bl->type==BL_MOB && (struct mob_data *)bl)
		...
		return &((struct pet_data *)bl)->opt2;
	return 0;
}
short *status_get_opt3(struct block_list *bl)
{
	...
	if(bl->type==BL_MOB && (struct mob_data *)bl)
		...
		return &((struct pet_data *)bl)->opt3;
	return 0;
}
short *status_get_option(struct block_list *bl)
{
	...
	if(bl->type==BL_MOB && (struct mob_data *)bl)
		...
		return &((struct pet_data *)bl)->option;
	return 0;
}
...
int status_change_start(struct block_list *bl,int type,int val1,int val2,int val3,int val4,int tick,int flag)
{
	...
	switch(type){	/* 異常の種類ごとの処理 */
		...
		case SC_RACEDRAGON:
			if(sd){
				char message[64];
				sprintf(message,"%s種族になりました",race_name[type-SC_RACEUNKNOWN]);
		...
		case SC_FLAMELAUNCHER:		/* フレームランチャー */
			status_encchant_eremental_end(bl,SC_FLAMELAUNCHER);
			break;
		case SC_FROSTWEAPON:		/* フロストウェポン */
			status_encchant_eremental_end(bl,SC_FROSTWEAPON);
			break;
		case SC_LIGHTNINGLOADER:	/* ライトニングローダー */
			status_encchant_eremental_end(bl,SC_LIGHTNINGLOADER);
			break;
		case SC_SEISMICWEAPON:		/* サイズミックウェポン */
			status_encchant_eremental_end(bl,SC_SEISMICWEAPON);
			break;
		case SC_DARKELEMENT:		/* 闇属性 */
			status_encchant_eremental_end(bl,SC_DARKELEMENT);
			break;
		case SC_ATTENELEMENT:		/* 念属性 */
			status_encchant_eremental_end(bl,SC_ATTENELEMENT);
			break;
		case SC_UNDEADELEMENT:	/* 不死属性 */
			status_encchant_eremental_end(bl,SC_UNDEADELEMENT);
			break;
...
int status_change_end_by_jumpkick( struct block_list* bl)
{
	...
	if(bl->type!=BL_PC && bl->type!=BL_MOB && bl->type!=BL_PET) {	//added by フェルシア
		if(battle_config.error_log)
...
//						sd->status.manner = 0;
			...
			case SC_SELFDESTRUCTION:		/* 自爆 */
				{
					//自分のダメージは0にして
					struct mob_data *md;
					if(bl->type == BL_MOB && (md=(struct mob_data*)bl) && md->hp > 0) {
						skill_castend_damage_id(bl, bl,sc_data[type].val2,sc_data[type].val1,gettick(),0 );
						// この時点でblは無効になっているので、すぐに戻る必要がある
						return 0;
					}
				}
				break;
		...
		if(bl->type==BL_PC // && !(flag&4) //これを追加すると利点が増えると思うけど
		 	&& (calc_flag || ((struct map_session_data *)bl)->auto_status_calc_pc[type]==1))
		{
...
int status_change_timer(int tid, unsigned int tick, int id, int data)
{
	...
	//
	if(sd && sd->eternal_status_change[type]>0)
	{
		sc_data[type].timer=add_timer(	/* タイマー再設定 */
...
int status_change_soulstart(struct block_list *bl,int val1,int val2,int val3,int val4,int tick,int flag)
{
	...
	switch(s_class.job){
		case 15:
			status_change_start(bl,SC_MONK,val1,val2,val3,val4,tick,flag);
			break;
		case 25:
			status_change_start(bl,SC_STAR,val1,val2,val3,val4,tick,flag);
			break;
		case 16:
		 	status_change_start(bl,SC_SAGE,val1,val2,val3,val4,tick,flag); 
		 	break;
		case 14:
			status_change_start(bl,SC_CRUSADER,val1,val2,val3,val4,tick,flag);
			break;
		case 9:
			status_change_start(bl,SC_WIZARD,val1,val2,val3,val4,tick,flag);
			break;
		case 8:
			status_change_start(bl,SC_PRIEST,val1,val2,val3,val4,tick,flag);
			break;
		case 17:
			status_change_start(bl,SC_ROGUE,val1,val2,val3,val4,tick,flag);
			break;
		case 12:
			status_change_start(bl,SC_ASSASIN,val1,val2,val3,val4,tick,flag);
			break;
		case 27:
			status_change_start(bl,SC_SOULLINKER,val1,val2,val3,val4,tick,flag);
			break;
		case 7:
			status_change_start(bl,SC_KNIGHT,val1,val2,val3,val4,tick,flag);
			break;
		case 18:
			status_change_start(bl,SC_ALCHEMIST,val1,val2,val3,val4,tick,flag);
			break;
		case 19:
		case 20:
			status_change_start(bl,SC_BARDDANCER,val1,val2,val3,val4,tick,flag);
			break;
		case 10:
			status_change_start(bl,SC_BLACKSMITH,val1,val2,val3,val4,tick,flag);
			break;
		case 11:
			status_change_start(bl,SC_HUNTER,val1,val2,val3,val4,tick,flag);
			break;
		case 23:
			status_change_start(bl,SC_SUPERNOVICE,val1,val2,val3,val4,tick,flag);
			break;
		default:
			if(s_class.upper==1 && s_class.job>=1 && s_class.job<=6)
				status_change_start(bl,SC_HIGH,val1,val2,val3,val4,tick,flag);
			break;
	}
