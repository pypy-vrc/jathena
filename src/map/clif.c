...
unsigned char grounddrift_unit_id[] = {0xc2,0xbe,0xc0,0xbf,0xc1};	//グラウンドドリフト個別エフェクト
...
static struct linkdb_node *clearchar_list;
...
static int clif_clearchar_delay_sub(int tid,unsigned int tick,int id,int data)
{
	...
	linkdb_erase(&clearchar_list, (void*)tid);
	aFree(bl);
...
int clif_clearchar_delay(unsigned int tick,struct block_list *bl,int type)
{
	...
	tid = add_timer(tick,clif_clearchar_delay_sub,(int)tmpbl,type);
	linkdb_insert(&clearchar_list, (void*)tid, tmpbl);
...
static int clif_hom0078(struct homun_data *hd,unsigned char *buf)
{
	...
	WBUFB(buf,49)=0;
	WBUFB(buf,50)=0;
	WBUFW(buf,52)=status_get_lv(&hd->bl);
...
#if PACKETVER < 4
	if(sd
	&& (type == LOOK_WEAPON || type == LOOK_SHIELD)
 ...
 *------------------------------------------
 */
void clif_showvendingboard(struct block_list* bl, char *message, int fd)
{
	unsigned char buf[128];
 ...
 *------------------------------------------
 */
void clif_vendinglist(struct map_session_data *sd, int id, struct vending *vending)
{
...
/*==========================================
 * マリオネットの赤い糸(現在ディボで代用中)
 *------------------------------------------
 */
void clif_marionette(struct map_session_data *sd, int target)
{
	unsigned char buf[56];
 ...
 *------------------------------------------
 */
void clif_bladestop(struct block_list *src, struct block_list *dst, int flag)
{
	unsigned char buf[32];
 ...
 *------------------------------------------
 */
static void clif_guild_explusionlist(struct map_session_data *sd)
{
	int fd;
	int i,c;
	struct guild *g;
...
void clif_send_packet(struct map_session_data *sd, const char *message)
{
	...
	if((n=sscanf(message, "%x %d %d %d %d %d", &packet,&p[0],&p[1],&p[2],&p[3],&p[4]))==0)
		return;

	WFIFOW(fd,0) = packet;
	len=packet_db[packet].len;
	...
	if(len>255){
		...
		for(i=0;i+2<len;i++)
			if(i<5 && i<n-1)	WFIFOB(fd,2+i)= p[i];
			else			WFIFOB(fd,2+i)=(p[i]=atn_rand()%256);

		printf("clif_send_packet_test packet:%x len:%d\nSend:",packet,len);
		for(m=0;m+2<len;m++) printf("%3d",m+3);
		printf("\nRead:");
		for(m=0;m+2<len;m++) printf("%3x",p[m]);
		printf("\n");
		WFIFOSET(fd,len);
	}else if(len==-1){		// 可変長
		if(n==1 || p[0]<=0){
			printf("clif_send_packet_test packet:%x len=-1 input len @packet len x1 x2...\n",packet);
			return;
		}
		WFIFOW(fd,2) = (len=p[0]);
		for(i=0;i+4<len;i++)
			if(i<5 && i<n-2)	WFIFOB(fd,4+i)= p[i];
			else			WFIFOB(fd,4+i)=(p[i]=atn_rand()%256);

		printf("clif_send_packet_test packet:%x len:%d\nSend:",packet,len);
		for(m=0;m+4<len;m++) printf("%3d",m+3);
		printf("\nRead:");
		for(m=0;m+4<len;m++) printf("%3x",p[m]);
		printf("\n");
		WFIFOSET(fd,len);
	}
...
static void clif_parse_WalkToXY(int fd,struct map_session_data *sd, int cmd)
{
	...
	if(sd->ud.skilltimer != -1 && pc_checkskill(sd,SA_FREECAST) <= 0) // フリーキャスト
		return;

	if(sd->chatID)
		return;

	if(sd->ud.canmove_tick > gettick())
		return;
	...
	if((sd->opt1 > 0 && sd->opt1 != 6) ||
		...
		sd->sc_data[SC_RUN].timer != -1		 || //駆け足
		sd->sc_data[SC_HIGHJUMP].timer != -1 ||	//ハイジャンプ
		(sd->sc_data[SC_GRAVITATION_USER].timer != -1 && battle_config.player_gravitation_type < 2) ||	//グラビテーションフィールド使用者
		sd->sc_data[SC_MADNESSCANCEL].timer!=-1 ||	//マッドネスキャンセラー状態は移動出来ない
		sd->sc_data[SC_CLOSECONFINE].timer!=-1 ||
		sd->sc_data[SC_HOLDWEB].timer!=-1 || //ホールドウェブ
		(battle_config.hermode_no_walking && sd->sc_data[SC_DANCING].timer !=-1 &&
		 sd->sc_data[SC_DANCING].val1 == CG_HERMODE) ||
		(sd->sc_data[SC_LONGINGFREEDOM].timer == -1 && sd->sc_data[SC_DANCING].timer !=-1 &&
		 sd->sc_data[SC_DANCING].val4) || //合奏スキル演奏中は動けない
		(sd->sc_data[SC_LONGINGFREEDOM].timer == -1 && sd->sc_data[SC_DANCING].timer !=-1 &&
		 sd->sc_data[SC_DANCING].val1 == CG_MOONLIT) //月明りは動けない
		) //
		return;

	// 単独合奏時に動けない設定
	if (!battle_config.player_skill_partner_check && !(battle_config.sole_concert_type & 1) &&
		sd->sc_data[SC_LONGINGFREEDOM].timer == -1 && sd->sc_data[SC_DANCING].timer != -1)
	{
		if(sd->sc_data[SC_DANCING].val1>=BD_LULLABY && sd->sc_data[SC_DANCING].val1<=BD_SIEGFRIED)
			return;
	}
	...
	if ((sd->sc_data[SC_BASILICA].timer!=-1 && sd->sc_data[SC_BASILICA].val2==sd->bl.id) ||
		...
		return;	// バジリカもしくはゴスペルを張っている人は動けない
...
static void clif_parse_GetCharNameRequest(int fd,struct map_session_data *sd, int cmd)
{
	...
	case BL_PC: {
			...
			if ((battle_config.show_always_party_name && p != NULL) ||
			    g != NULL) {
				// ギルド所属ならパケット0195を返す
				WFIFOW(fd, 0) = 0x195;
				if (p)
...
static void clif_parse_TakeItem(int fd,struct map_session_data *sd, int cmd)
{
	...
	if( sd->npc_id!=0 || sd->vender_id != 0 || sd->deal_mode != 0 || sd->opt1 > 0 || sd->chatID ||
		...
		sd->sc_data[SC_HIGHJUMP].timer!=-1 ||     //ジャンプ中拾えない
		sd->sc_data[SC_BLADESTOP].timer!=-1)) ) { //白刃取り
	...
	fitem = (struct flooritem_data*)map_id2bl(RFIFOL(fd,GETPACKETPOS(cmd,0))); // map_object_id
...
static void clif_parse_DropItem(int fd,struct map_session_data *sd, int cmd)
{
	...
	if( sd->npc_id != 0 || sd->vender_id != 0 || sd->deal_mode != 0 || sd->opt1 > 0 ||
		(sd->sc_data && (sd->sc_data[SC_AUTOCOUNTER].timer!=-1 || //オートカウンター
		sd->sc_data[SC_BLADESTOP].timer!=-1 || //白刃取り
		sd->sc_data[SC_HIGHJUMP].timer!=-1  || //投げ捨て禁止！
	...
	pc_dropitem(sd, RFIFOW(fd,GETPACKETPOS(cmd,0)) - 2, RFIFOW(fd,GETPACKETPOS(cmd,1))); // item_index, item_amount
...
static void clif_parse_UseItem(int fd,struct map_session_data *sd, int cmd)
{
	...
	if ((sd->npc_id != 0 && sd->npc_allowuseitem != 0 && sd->npc_allowuseitem != sd->status.inventory[idx].nameid) ||
		sd->special_state.item_no_use != 0 || sd->vender_id != 0 || sd->deal_mode != 0 || (sd->opt1 > 0 && sd->opt1 != 6) ||
...
static void clif_parse_UseSkillToId(int fd,struct map_session_data *sd, int cmd)
{
	...
	if (battle_config.check_sitting_player_using_skill && pc_issit(sd)) {
		// [Anti-hack] client can not send useskill packet if player is sitting. But hacker...
		return;
	}
	...
	//ホムスキル
	if(skillnum >= HOM_SKILLID && skillnum < MAX_HOM_SKILLID && (hd=sd->hd) && (lv=homun_checkskill(hd,skillnum))){
		hd = sd->hd;
		if(skilllv > lv) skilllv = lv;
		if(hd->ud.skilltimer != -1 && skillnum != SA_CASTCANCEL) return;
		else if(DIFF_TICK(tick , hd->ud.canact_tick) < 0 || DIFF_TICK(tick , hd->homskillstatictimer[skillnum-HOM_SKILLID]) < 0) return;
	...
	else if(skillnum < 0 || skillnum >= MAX_SKILL_ID) {
		// ギルドスキルなのでstatic timer の判定外
	}
	else if(DIFF_TICK(tick , sd->ud.canact_tick) < 0 || DIFF_TICK(tick , sd->skillstatictimer[skillnum]) < 0) {
...
static void clif_parse_UseSkillToPos(int fd,struct map_session_data *sd, int cmd)
{
	...
	if (battle_config.check_sitting_player_using_skill_p && pc_issit(sd)) {
		// [Anti-hack] client can not send useskill packet if player is sitting. But hacker...
		return;
	}
...
static void clif_parse_MoveToKafra(int fd,struct map_session_data *sd, int cmd)
{
	...
	if(sd->state.storage_flag==1)
		storage_guild_storageadd(sd, item_index, RFIFOL(fd,GETPACKETPOS(cmd,1))); // item_amount
	else if(sd->state.storage_flag==0)
		storage_storageadd(sd, item_index, RFIFOL(fd,GETPACKETPOS(cmd,1))); // item_amount
 ...
 *------------------------------------------
 */
static void clif_parse_GuildInvite(int fd,struct map_session_data *sd, int cmd)
{
...
static void clif_parse_GMReqNoChat(int fd,struct map_session_data *sd, int cmd)
{
	...
	if(sd->sc_data && sd->sc_data[SC_RUN].timer!=-1)
		return;
	//念のため走り高跳び中も
	if(sd->sc_data && sd->sc_data[SC_HIGHJUMP].timer!=-1)
		return;
...
static void clif_parse_SendMail(int fd,struct map_session_data *sd, int cmd)
{
	...
	if(RFIFOW(fd,2)<70)		// 短すぎ、おかしい
		return;

	bodylen = RFIFOW(fd,2)-69;
	rd = map_nick2sd(RFIFOP(fd,4));
	...
	memcpy(md.receive_name,RFIFOP(fd,4),24);
	snprintf(md.title,40,"%s",RFIFOP(fd,28));

	if(bodylen > sizeof(md.body))
		return;

	memcpy(md.body,RFIFOP(fd,68),bodylen);
...
static void clif_parse_MailWinOpen(int fd,struct map_session_data *sd, int cmd)
{
	int flag;
	nullpo_retv(sd);
	flag = RFIFOL(fd,GETPACKETPOS(cmd,0));
...
/*static void clif_parse_BabyRequest(int fd,struct map_session_data *sd, int cmd)
{
	...
	tsd = (struct map_session_data*)map_id2sd(char_id);
...
{
	if(clearchar_list) {
		struct linkdb_node *node = (struct linkdb_node *)clearchar_list;
		while(node) {
			aFree(node->data);
			node = node->next;
		}
		linkdb_final(&clearchar_list);
	}
