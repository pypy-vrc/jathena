...
static int atmobsearch_sub(struct block_list *bl,va_list ap)
{
	...
	if(md && fd && (mob_id==-1 || (mob_id==-2 && status_get_mode(&md->bl)&0x20) || (md->class==mob_id))){
		snprintf(output, sizeof output, msg_txt(94),
...
/*	if ((pl_sd = map_nick2sd(character)) != NULL) {
		...
		snprintf(output, sizeof output, msg_txt(4), character);
		clif_displaymessage(fd, output);
	} else {
		clif_displaymessage(fd, msg_txt(3));
	}
	...
	const char* command, const char* message)
{
	...
	if (hp == 0 && sp == 0) {
		...
		if (hp > 0 && hp > sd->status.max_hp - sd->status.hp)
			hp = sd->status.max_hp - sd->status.hp;
		else if (hp < 0 && hp < -sd->status.hp + 1)
			hp = -sd->status.hp + 1;
		if (sp > 0 && sp > sd->status.max_sp - sd->status.sp)
			sp = sd->status.max_sp - sd->status.sp;
		else if (sp < 0 && sp < -sd->status.sp)
			sp = -sd->status.sp;
	}
...
/*
	if ((pl_sd = map_nick2sd(character)) != NULL) {
		if (pc_isGM(sd) > pc_isGM(pl_sd)) {
			...
			snprintf(output, sizeof output, msg_txt(46), character);
			clif_displaymessage(fd, output);
		}
	} else {
		clif_displaymessage(fd, msg_txt(3));
	...
	const char* command, const char* message)
{
	...
	if (sscanf(message, "%s %99s", mob_name , map_name) < 0)
		return -1;

	if ((mob_id = atoi(mob_name)) == 0)
		 mob_id = mobdb_searchname(mob_name);
	if(mob_id !=-1 && mob_id !=-2 &&(mob_id <= 1000 || mob_id >= 2000)){
...
//				strcpy(mob_name,mob_db[mob_id].name);	// --en--
	...
	snprintf(output, sizeof output, msg_txt(92),
		mob_name, map[map_id].name);
	clif_displaymessage(fd, output);
 ...
 *------------------------------------------
 */
int atcommand_vars_sub(const int fd,struct map_session_data* sd,char* vars,char* name,char* str,int flag)
{
	int element=0;
	char *p,*output;
	struct map_session_data *pl_sd=NULL;
	...
	if(p) {
		...
		if(element < 0 || element >= 128)
			return -1;
		memset(p,'\0',strlen(p)+1);	// []部分を削る
	}
	output = script_operate_vars(pl_sd, name, vars, element, str, flag);
	...
	const char* command, const char* message)
{
	...
	return atcommand_vars_sub(fd, sd, vars, name, NULL, 1);
}
	...
	const char* command, const char* message)
{
	...
	if(strchr(message,'\"')) {
		int i=0;
		char *buf = (char *)aCalloc(204,sizeof(char));	// '\"'+str+'\"'+' '+name+'\0' = 204
		char *p;
	...
	return atcommand_vars_sub(fd, sd, vars, name, str, 0);
}
	...
	const char* command, const char* message)
{
	...
	ret = sscanf(message, "%d %d",&skillid,&skilllv);
	if(ret<1)
		return -1;
	else if(ret==1)
...
int atcommand_cloneskill2(
	...
	const char* command, const char* message)
{
	int cloneskilllv=0,skillid=0,skilllv=0,ret;
	...
	const char* command, const char* message)
{
	...
	if ((mob_id = mobdb_searchname(message)) == 0)
		mob_id = atoi(message);
	...
	for (i = 0; i < ITEM_DROP_COUNT; i++) {
		...
		if (mob->dropitem[i].p > 0) {
			sprintf(output2, " - %s  %02.02f%%", item_data->jname, (float)mob->dropitem[i].p / 100);
	...
	const char* command, const char* message)
{
	...
	if (level >= 1) {
		for (i = 1; i <= level; i++){
			hd->status.base_exp += homun_nextbaseexp(hd)-hd->status.base_exp;
	...
	const char* command, const char* message)
{
	...
	nullpo_retr(-1, sd);
	if (sscanf(message, "%d", &class) < 1)
		return -1;
	if(sd->hd)
		sd->hd->view_class = class;
	return 0;
}
/*==========================================
 * 
	...
	const char* command, const char* message)
{
	...
	if(sd->hd)
	{
		...
		if(sscanf(message, "%d", &evo_class)<1)
		{
			...
			if(homun_db[class].evo_class){
				sd->hd->status.class = homun_db[class].evo_class;
				class = homun_db[class].evo_class-HOM_ID;
				sd->hd->view_class = homun_db[class].view_class;
				homun_return_embryo(sd);
				homun_callhom(sd);
			}
		}else{
			class = evo_class-HOM_ID;
			sd->hd->status.class = evo_class;
			sd->hd->view_class = homun_db[class].view_class;
			homun_return_embryo(sd);
			homun_callhom(sd);
		}
