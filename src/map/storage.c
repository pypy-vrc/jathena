...
int storage_storageopen(struct map_session_data *sd)
{
	...
	if((stor = numdb_search(storage_db,sd->status.account_id)) != NULL) {
		...
		if(sd->status.guild_id>0 && (gstor = numdb_search(guild_storage_db,sd->status.guild_id)) &&
		   gstor->storage_status && sd->state.storage_flag)
...
void storage_storageadd(struct map_session_data *sd, int idx, int amount)
{
	...
	if (stor->storage_status == 1) { // storage open
		if (idx >= 0 && idx < MAX_INVENTORY) { // valid index
			if ((amount <= sd->status.inventory[idx].amount) && (amount > 0)) { // valid amount
...
void storage_storageaddfromcart(struct map_session_data *sd, int idx, int amount)
{
	...
	if (stor->storage_status == 1) { // storage open
		if (idx >= 0 && idx < MAX_CART) { // valid index
			if ((amount <= sd->status.cart[idx].amount) && (amount > 0) && itemdb_isstorageable(sd->status.cart[idx].nameid)) { // valid amount
...
void storage_storagegettocart(struct map_session_data *sd, int idx, int amount)
{
	...
	if(stor->storage_status == 1) { // storage open
		if (idx >= 0 && idx < MAX_STORAGE) { // valid index
			if ((amount <= stor->storage[idx].amount) && (amount > 0) && itemdb_iscartable(stor->storage[idx].nameid)) { // valid amount
...
int storage_guild_storageopen(struct map_session_data *sd)
{
	...
	if((gstor = numdb_search(guild_storage_db,sd->status.guild_id)) != NULL) {
		...
		if((stor = numdb_search(storage_db,sd->status.account_id))!=NULL && stor->storage_status)
