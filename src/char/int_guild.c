...
void guild_txt_final(void) {
	...
	if(castle_db)
		numdb_final(castle_db,guildcastle_db_final);
}
...
int guild_sql_new(struct guild *g) {
	...
	sprintf(
		tmp_sql,
		"INSERT INTO `%s` (`guild_id`,`name`,`guild_lv`) VALUES ('%d','%s','1')",
		guild_db_,g->guild_id,strecpy(t_name,g->name)
	);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
...
// ギルド城データのセーブ用
int guildcastle_sync_sub(void *key,void *data,va_list ap)
{
	char line[16384];
	FILE *fp;
...
int guildcastle_sync(void) {
	...
	numdb_foreach(castle_db,guildcastle_sync_sub,fp);
	...
	int type,const char *data,int len)
{
	...
	case GMI_EXP:
		{	// EXP
			int exp,oldexp=g2.member[i].exp;
			exp=g2.member[i].exp=*((unsigned int *)data);
			g2.exp+=(exp-oldexp);
