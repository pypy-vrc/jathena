...
int mail_store_mail(int char_id,struct mail_data *md)
{
	...
	fprintf(fp,"%d,%d\t%s\t%s\t%s\t%d\t%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\t%d\t%d\t",
		...
		(unsigned int)md->times,md->body_size);
...
int mail_save_mail(int char_id,int store,struct mail_data *md[MAIL_STORE_MAX])
{
	...
	while(n<store && md[n+1]){
		...
		fprintf(fp,"%d,%d\t%s\t%s\t%s\t%d\t%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\t%d\t%d\t",
			...
			(unsigned int)md[n]->times, md[n]->body_size);
	...
	if((fp=fopen(filename,"r"))!=NULL){
		while(i+1<store && fgets(line,65535,fp) && i+1<MAIL_STORE_MAX){
			...
			if(n==20){
				...
				md[i]->times		= tmp_int[14];
				md[i]->body_size	= tmp_int[15];
...
int mapif_parse_OpenMailBox(int fd)
{
	...
	const struct mmo_charstatus *rd;
	const struct mail *m;
	struct mail_data *md[MAIL_STORE_MAX];
	...
	if(m){
		mail_read_mail(char_id,m->store,md);
		mapif_send_mailbox(fd,rd->name,m->store,md);
		mail_free(md,m->store);
	}
	else
		mapif_send_mailbox(fd,rd->name,0,NULL);
	return 0;
}
int mapif_parse_SendMail(int fd)
{
	const struct mmo_charstatus *sd,*rd;
	struct mail_data md;
	...
	if(receive_id < 0){		// 受け取る人が存在しません
		mapif_mail_res(fd,sd->account_id,1);
	}else{
		const struct mail *m;
		struct mail m2;
		...
		if(rd==NULL) return 0;
		if(sd->account_id == rd->account_id){	// 同じアカウントには送れない
			mapif_mail_res(fd,sd->account_id,1);
			return 0;
		}
		// 保存だの何だの
		...
		if(m){
			...
			mail_new(rd->account_id,rd->char_id);
			md.mail_num = 1;
		}
		mail_store_mail(receive_id,&md);				// 保存
		mapif_mail_res(fd,sd->account_id,0);			// 送信成功
