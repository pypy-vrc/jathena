...
int homun_tostr(char *str,struct mmo_homunstatus *h)
{
	...
	*(str_p++)='\t';
	...
	for(i=0;i<MAX_HOMSKILL;i++)
		if(h->skill[i].id && h->skill[i].flag!=1 && h->skill[i].flag!=CLONE_SKILL_FLAG){
...
//	printf("sscanf homun main info\n");
	...
	for(i=0;str[next] && str[next]!='\t';i++){
		...
		if(set!=2)
			return 0;
		h->skill[tmp_int[0]-HOM_SKILLID].id = tmp_int[0];
		h->skill[tmp_int[0]-HOM_SKILLID].lv = tmp_int[1];
		next+=len;
		if(str[next]==' ')
			next++;
	}
...
int homun_txt_save(struct mmo_homunstatus* p2) {
	...
	if(p1 == NULL) {
		...
		numdb_insert(homun_db,p1,p2->homun_id);
	}
...
int  homun_sql_save(struct mmo_homunstatus* p2) {
	...
	if(memcmp(p1->skill, p2->skill, sizeof(p1->skill)) ) {
		...
		sep = ' ';
		for(i=0;i<MAX_HOMSKILL;i++) {
			if(p2->skill[i].id && p2->skill[i].flag!=1 && p2->skill[i].flag!=CLONE_SKILL_FLAG){
