// $Id: login.c,v 1.2 2005/09/06 19:29:10 running_pinata Exp $
...
static int login_version = -1, login_type = -1;
...
static void login_httpd_account(struct httpd_session_data *sd,const char* url) {
	...
	do {
		if(httpd_get_method(sd) != HTTPD_METHOD_POST) {
			// POST以外お断り
			msg = "Illeagal request."; break;
		}
		if(!new_account_flag) {
			msg = "Now stopping to create accounts."; break;
		}
		...
		if(i != userid_len) {
			msg = "Illeagal character found in UserID."; break;
		}
		...
		if(i != passwd_len) {
			msg = "Illeagal character found in Password."; break;
		}
